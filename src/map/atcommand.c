...
ACMD(storage)
{
	...
	if(map->list[sd->bl.m].flag.nostorage){
...
ACMD(guildstorage)
{
	if(map->list[sd->bl.m].flag.noguildstorage){
		clif->messagecolor_self(sd->fd, COLOR_WHITE, msg_sd(sd, 49));
		return true;
	}
...
#endif
		{ MAP_ALDEBARAN,   168, 112, 3 }, //  6 = Aldebaran
...
#define SETFLAG( cmd ) do { \
	...
	if (!message || !*message || (sscanf(message, "%99s %hd", flag_name, &flag) < 1)) {
		...
		CHECKFLAG(notomb);            CHECKFLAG(nocashshop);         CHECKFLAG(nostorage);    CHECKFLAG(noguildstorage);
		clif->message(sd->fd," ");
	...
	SETFLAG(notomb);            SETFLAG(nocashshop);         SETFLAG(nostorage);    SETFLAG(noguildstorage);
	...
	clif->message(sd->fd,"guildlock, src4instance, notomb, nocashshop, nostraoge, noguildstorage");
#undef CHECKFLAG
#undef SETFLAG
		...
		for(i = 0; i < map->list[sd->bl.m].zone->disabled_commands_count; i++) {
			if( info->func == map->list[sd->bl.m].zone->disabled_commands[i]->cmd ) {
				if (pc_get_group_level(sd) < map->list[sd->bl.m].zone->disabled_commands[i]->group_lv) {
					clif->messagecolor_self(sd->fd, COLOR_WHITE, msg_sd(sd, 882));
					return true;
				} else {
