...
bool map_zone_mf_cache(int m, char *flag, char *params) {
	...
	if (!strcmpi(flag, "nosave")) {
		...
		if( state && map->list[m].flag.nostorage )
			...
			if( state )
				map_zone_mf_cache_add(m, "nostorage\toff");
			else if( map->list[m].flag.nostorage )
				map_zone_mf_cache_add(m, "nostorage");
		}
	} else if (!strcmpi(flag, "noguildstorage")) {
		if( state && map->list[m].flag.noguildstorage )
			;/* nothing to do */
		else {
			if( state )
				map_zone_mf_cache_add(m, "noguildstorage\toff");
			else if( map->list[m].flag.noguildstorage )
				map_zone_mf_cache_add(m, "noguildstorage");
		}
	}
