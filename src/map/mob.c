...
int mob_dead(struct mob_data *md, struct block_list *src, int type) {
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP; i++)
		{
			...
			// Increase drop rate if the SC_WIDEDROPBOOST is activated
			//if (sd && sd->sc.data[SC_WIDEDROPBOOST]) // now rig the drop rate to never be over 90% unless it is originally >90%.
			//	drop_rate = max(drop_rate, cap_value((int)(0.5 + drop_rate * (sd->sc.data[SC_WIDEDROPBOOST]->val1) / 100.), 0, 9000));
